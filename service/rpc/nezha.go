...
func (s *NezhaHandler) ReportGeoIP(c context.Context, r *pb.GeoIP) (*pb.GeoIP, error) {
	...
	if geoip.IP.IPv4Addr != "" {
		ipv4 = geoip.IP.IPv4Addr
	}
	if geoip.IP.IPv6Addr != "" {
		ipv6 = geoip.IP.IPv6Addr
	}
	netIPv4 := net.ParseIP(ipv4)
	netIPv6 := net.ParseIP(ipv6)
	locationIpv4, err := geoipx.Lookup(netIPv4)
	if err != nil {
		log.Printf("NEZHA>> geoip.Lookup ipv4: %v", err)
	}
	locationIpv6, err := geoipx.Lookup(netIPv6)
	if err != nil {
		log.Printf("NEZHA>> geoip.Lookup ipv6: %v", err)
	}
	geoip.Ipv4Geo = locationIpv4
	geoip.Ipv6Geo = locationIpv6
