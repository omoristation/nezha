...
func getServerStat(withPublicNote, authorized bool) ([]byte, error) {
	v, err, _ := requestGroup.Do(fmt.Sprintf("serverStats::%t", authorized), func() (interface{}, error) {
		...
		for _, server := range serverList {
			var countryCode,ipv4,ipv4Geo,ipv6,ipv6Geo string //diy
			if server.GeoIP != nil {
				countryCode = server.GeoIP.CountryCode
				ipv4 = server.GeoIP.IP.IPv4Addr //diy
				ipv4Geo = server.GeoIP.Ipv4Geo //diy
				ipv6 = server.GeoIP.IP.IPv6Addr //diy
				ipv6Geo = server.GeoIP.Ipv6Geo //diy
			}
